...
//#include <glm/glm.hpp>  //!< Include if transformations are needed.
...
bool Win32OpenGL::CreateGLContext(HWND hWnd) //!< Utilises HWND not HDC so we can generate message boxes
{
	HDC hdc = GetDC(hWnd);

	PIXELFORMATDESCRIPTOR pfd; //!< Ask for a pixel format
	...
	int nPixelFormat = ChoosePixelFormat(hdc, &pfd); //!< Choose pixel format.
